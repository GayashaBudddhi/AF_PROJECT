...
class ViewPayment extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:8081/workshoppayments/')
            .then(response => {
    ...
    deletePayment(e, id){
        axios.delete(`http://localhost:8081/workshoppayments/delete/${id}`)
            .then(response => {
                ...
                {this.state.workshoppayments.length > 0 && this.state.workshoppayments.map((item,index) => (
                    <div key={index} className="card mb-3">
                        <div className="p-3">
                            ...
                            <h6>Payment Amount: {item.depositedAmount}</h6>
